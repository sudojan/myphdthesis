...
Due to the through-going charged particle, the medium gets polarized and creates a signal. These signals are emitted coherently when the charged particle propagates faster than the speed of light in this medium, creating a Cherenkov cone with an opening angle of $\cos \theta = 1/\beta n$ similar to a hypersonic cone of an Airforce jet.
The Frank-Tamm formula \cite{FrankTamm37} describing the emitted Cherenkov photons has a $1/\lambda^2$ dependency, with the wavelength $\lambda$, and is therefore UV-divergent.
...
The geomagnetic effect describes the the separation of positrons and electrons during an electromagnetic cascade due to the geomagnetic field.
...
Since there are only atomic electrons and no positrons, these electrons of a medium gets accelerated by the shower particles and the shower front gets charged negatively leaving the positively charged ions behind.
...
Both effect just measurable because the particle cascades propagate faster than the speed of lights in the medium thus producing a coherent radio signal at the Cherenkov angle.
For air shower, the radio signal is mainly produced by the geomagnetic effect while for more dense media, like ice, the Askaryan effect produces the dominant radio signal.
Only for the highest energetic particle showers ($>\si{EeV}$) producing a sufficient amount of electrons and positrons, this radio signal can be detected.
...
Another type is the luminescence light which is used in searches of magnetic monopoles with neutrino telescopes, where the radio-luminescence induced by these highly ionizing particles has been become a field of research \cite{Pollmann19Luminescence}.
...
The detection technique of both types of gamma telescope designs are illustrated in \figref{fig:detect_swgo}.
...
Compared to the closed tanks of EAS-Arrays with a duty cycle of nearly \SI{100}{\%}, IACTs can only operate at clear nights limiting their duty cycle to \SI{20}{\%}.
\begin{figure}
    \centering
...
A transit between a cosmic-ray induced muon detector and a neutrino detector is the NEutrino VOda (Water) Detector, NEVOD \cite{NEVOD15} located inside a building  at the MePhI in Moscow.
...
So far, only experiments at the surface have been discussed measuring muonic shower component as part of the signal or the main signal.
...
This peripheral detector type is used, since the absorption length of the Cherenkov radiation is larger then the detector size.
...
Further neutrino telescopes using the detection principle like IceCube are the ANTARES/Km3Net \cite{ANTARES11, KM3Net16} experiment in the Mediterranean sea, the Baikal/GVD in lake Baikal \cite{Baikal97, GVD19} and the P-ONE experiment in the Bassin bay in front of Vancouver \cite{PONE20}.
...
Although the detection media is always water-based, the propagation of the Cherenkov light mainly described by the scattering and absorption differ significantly, as shown in \tabref{tab:len_abs_scat}.
...
Also with the planned IceCube-Gen2 detector increasing the size by a factor of ten \cite{IceCube20Gen2}the flux of the highest energetic neutrinos is too low to be detectable.
...
Therefore these detectors can be placed sparsely and cover 
...
On the other side it works as a veto for IceCube to distinguish filter down-going neutrinos from atmospheric muon events as the neutrinos should not be seen in IceTop.

There are currently negotiations for an extension of IceCube named IceCube-Gen2 \cite{IceCube20Gen2}.
...
In contrast to IceCube, the Strings in Gen2 will be organized on a sunflower structure avoiding corridors how muons can sneak inside the inner volume and mimic a starting event.
...
Only neutrinos can produce starting events and stopping events can only be produced by a huge stochastic loss, which in unlikely or by low energetic muons.
Most-often, muons propagate through the detector producing a long path along their track.
There is however the the special case of a corner clipper, that can mimic a cascade-like event at the edge of the detector.
...
Another approach to use atmospheric muons are \textbf{stopping muons} \cite{Hoinka17Master}.
...
\textbf{Up-going muon tracks} can just be neutrino induced muons as muons cannot propagate large distances through the earth.
...
After the selection the energy and directional reconstruction is the remaining step before analysing the desired event sample.
A descent overview of the standard reconstruction methods is given in \cite{AMANDA2004Reco, IceCube2014Ereco}.
...
For starting tracks, the energy resolution of the muons and neutrinos increases, due to the additional information of the hadronic cascade at the vertex.
